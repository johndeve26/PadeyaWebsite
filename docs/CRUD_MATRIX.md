...
|---|---|---|---|---|---|---|---|---|---|---|
| Users | `users` | Register | Self me + admin list/detail | `PATCH /users/me`; admin account-status / under-review / suspend / ban / selective restrictions | Soft status (`account_status`); force-delete soft EOL (`deleted`) only when already `suspended`; hard delete **blocked** (`405`) | Soft EOL / No hard | `/register`, `/login`, `/admin/users*` | `POST .../status`, `.../restrictions*`, `.../suspend\|unsuspend`, `POST .../force-delete`, `DELETE .../admin/{id}` → 405; Activity drill-down `GET .../activity/{kind}` | **complete** | Role assign still seed/ops; Activity detail audited + finance-gated |
| User gender | `users.gender` + `users.gender_visibility` | Register (required explicit selection) | Owner `/me` always; others via `can_view_gender` only | Owner `PATCH /users/me` (`gender`, `gender_visibility`) | Soft (nullable gender for legacy); visibility default `connections_only` | N/A | Signup, passport/host settings, passport/connect/messaging badges, admin detail | `app/users/gender.py`; migration `20260729_0148` | **complete** | Never inferred; hidden values omitted from API; org hosts suppress personal gender |
...
| Resource | Model | Create | Read | Update | Delete / lifecycle | Hard delete? | Frontend | Backend (key) | Status | Primary gap |
|---|---|---|---|---|---|---|---|---|---|---|
| Events | `events` | Host draft → submit **publishes** (+ soft flag) | Public/host/admin | PATCH (+ soft re-flag, optional `slug`); postpone dates (no re-flag) | pause/resume/postpone/cancel/complete; **auto-complete**; discard unused draft; admin **force-delete** (any status, reason); flag/reject; bulk pause/cancel/archive/delete | Host discard: draft/rejected no sales; admin force-delete: any | **10-step Event Studio**; `/host/events`; `/admin/events` (select + Deactivate/Cancel/Archive/Delete permanently) | `/events/*`; `POST /events/admin/{id}/force-delete` | **complete** | Force-delete cascades related rows — use for test cleanup |
...
| Public media assets | `public_media_assets`, `public_media_variants` | Created on public raster upload (avatar, event, blog, taxonomy, memories, host staging) | Variants via entity serializers (`banner_media`, `thumbnail_url`, …) | Replace = new immutable asset; entity points at new display URL | Soft `deleted_at` / `replaced_at`; orphan cleanup later | Prefer soft replace; never expose `source_key` | Shared FE `PublicMediaImage` + lightbox `enlargeSrc` | Upload endpoints return variant payload; legacy URL = display | **complete** | Controlled backfill CLI; historical rows keep legacy single URL until backfilled |
...
| Refund requests | `refund_requests` | Buyer | Buyer/support/finance | Escalate/review; optional `line_allocations` JSON | Close via decision | Never | `/dashboard/refunds`, `/support/refunds`, `/admin/refunds` | `/finance/refunds*` | **complete** | Buyer cancel open request; `requires_referral_refund_allocation` when mixed partial lacks lines |
| Referral attributions | `referral_attributions` | System after paid finalize | Aggregates | — | Append-only | Never | dashboards | finalize + reporting | partial | One winner per order item |
| Referral commission ledger | `referral_commission_entries` | System earn/reverse | Amb/host/admin | Reverse/adjust append-only | Reverse-only; never mutate earnings | Never hard | ambassador/host/admin referral UIs | ledger + finance refund | partial | Idempotent `referral-reversal:{event}:{earning}:{allocation}` |
...
| Legacy tiers | `legacy_tiers` | Seed | Admin; public via Legacy payloads | Admin PATCH | `is_active` | Soft | `/admin/legacy/tiers`, `/legacy` | `/legacy/admin/tiers*` | **ok** | Display uses configured tiers; no host self-edit |
| Legacy score history | `host_legacy_scores`, `host_legacy_score_history` | Recalc triggers | Admin history; host tier progress; public `legacy_trust` (safe fields) | Immutable history; score via recalc only | Never | Never | Admin legacy; host `/host/legacy/tier`; public Legacy summary | `/legacy/admin/hosts/...`, `/legacy/me/tier`, `/u/.../legacy` | **ok** | Public: display_score, provisional, evidence, next-tier, bands; no factor weights on public; see [LEGACY_SCORE.md](./LEGACY_SCORE.md) |
...
| Ambassador campaigns | `ambassador_campaigns` | Host or admin (`source=platform`); type `event_tickets` \| `event_merch` (v1) / `event\|merch\|host\|platform` (domain); commission + visibility + cookie window | Host own / admin all | PATCH + pause/resume/end; admin pause | End / archive (soft); no hard delete | Soft preferred | `/host/ambassadors/campaigns*`, `/admin/ambassadors/campaigns` | `/promos/campaigns*`, `/promos/admin/campaigns*` | partial | One open campaign per `(event, type)` v1; parent `program_id` links to `referral_programs` |
| Referral programs | `referral_programs` | Admin platform-wide; event-scoped backfill from campaigns | Admin list/detail; ambassadors see own enrollments | PATCH + activate/pause/close | Close/archive only (no hard delete with finance history) | Soft preferred | `/admin/ambassadors/programs` | `/promos/admin/referral-programs*` | partial | Platform scope: Pàdéyá-funded; event scope: host-funded via campaigns |
| Referral program rules | `referral_program_rules` | With program create/update | Admin | Upsert ticket/merch rules | Deactivate `is_active` | Soft | nested in program UI | nested in program APIs | partial | Unique `(program_id, product_type)` |
| Referral program exclusions | `referral_program_exclusions` | Admin on platform program | Admin | Replace/add | Soft remove row | Soft | program form | create payload `excluded_*_ids` | partial | Host or event exclusion |
| Ambassadors (platform-wide) | `ambassadors` (`program_kind=platform_wide`, nullable `host_id`) | Admin enroll/invite | Owner me + admin | Suspend/end | Soft suspend/end | Soft preferred | `/r/{code}`, ambassador dashboard | `/promos/admin/referral-programs/*/enrollments`, `/promos/referral/resolve/{code}` | partial | Code prefers Fan Passport username; tickets+merch by default (no host opt-in); dual stack with host enrollment; payer_type=platform |
...
| Resource | Model | Create | Read | Update | Delete / lifecycle | Hard delete? | Frontend | Backend (key) | Status | Primary gap |
|---|---|---|---|---|---|---|---|---|---|---|
| Blog posts | `blog_posts` (+ authors; studio: `studio_brief`/`studio_outline`/`faqs`/`content_version`; block editor: `content_document`/`content_document_version`/`editor_mode`/`hero_settings`; `blog_layout_templates`; `blog_reusable_sections`; `blog_revisions`; `blog_ai_operations`; `post_type_id`) | Admin (`admin.blog.create`) | Public published only; admin all (+ draft preview) | PATCH + autosave (content_version conflict 409); document PATCH (`content_document` authoritative in block mode); taxonomy assignment validates active terms (archived kept on existing posts) | Publish / unpublish / schedule / archive (publish creates revision; AI never auto-publishes) | Prefer archive; admin force-delete (`POST …/force-delete`, reason required, cascades comments/revisions) for test cleanup | `/blog*`, `/admin/blog*`, `/admin/analytics/blog` | `/api/v1/blog/*`, `/api/v1/admin/blog/*`, `/api/v1/admin/blog/posts/{id}/force-delete`, `/api/v1/admin/blog/posts/{id}/document*`, `/api/v1/admin/blog/layout-templates`, `/api/v1/admin/blog/reusable-sections`, `/api/v1/admin/blog/ai/*`, `/api/v1/analytics/admin/blog*` | **complete** | Legacy `cms_blog_posts` migrated; markdown + sanitize; Blog AI Studio; block editor (`content_document` authoritative when not legacy-only); explicit legacy conversion; derived `body`/`body_html`; see `docs/BLOG_CONTENT_AUTHORITY.md` |
| Blog categories | `blog_categories` (+ `blog_taxonomy_slug_redirects`) | Admin (`admin.blog.taxonomy.manage`) | Public active only; admin include_archived | PATCH name/description/SEO/sort; slug change requires `confirm_slug_change` + redirect | Archive / restore; usage count | Hard delete blocked (405) | `/admin/blog/taxonomies?tab=categories` (legacy `/admin/blog/categories` redirects) | `/admin/blog/categories*`, public `/blog/categories*` | **complete** | Flat only; `admin.blog.edit` selects terms but cannot mutate catalog; see `docs/BLOG_TAXONOMY.md` |
| Blog tags | `blog_tags` (+ slug redirects) | Admin (`admin.blog.taxonomy.manage`) | Public active only; admin include_archived | PATCH name/description/sort; slug change requires confirm + redirect | Archive / restore; usage count | Hard delete blocked (405) | `/admin/blog/taxonomies?tab=tags` | `/admin/blog/tags*`, public `/blog/tags*` | **complete** | Flat; assignment rejects newly selecting archived tags |
| Blog post types | `blog_post_types` | Admin (`admin.blog.taxonomy.manage`); system rows seeded by migration `20260729_0210` | Admin list (+ Studio pickers); no public hub | PATCH display name/description/sort; system `key` immutable | Archive / restore; usage count | Hard delete blocked (405) | `/admin/blog/taxonomies?tab=post-types` | `/admin/blog/post-types*` | **complete** | Identity = `id` + immutable `key`; name is label only; GET does not seed |
| Blog media roles | `blog_media_roles` | Admin (`admin.blog.taxonomy.manage`); system roles seeded by migration | Admin list; upload contexts | PATCH display name/description/sort; system `key` + `storage_folder` immutable via normal UI | Archive / restore (required roles blocked via `can_archive`); display usage approximate for inline/gallery | Hard delete blocked (405) | `/admin/blog/taxonomies?tab=media-roles` | `/admin/blog/media-roles*`, `POST /admin/blog/media/upload` | **complete** | Blog-only; event `MEDIA_TYPES` untouched; approximate usage never drives deletion |
...
| Date | Change |
|---|---|
| 2026-08-03 | Public media variants: shared processor + `public_media_assets`/`variants` (`20260803_0219`); upload paths emit thumbnail/card/display/full; FE helpers + lightbox enlargeSrc; docs `PUBLIC_MEDIA_VARIANTS.md` |
| 2026-08-02 | Admin events permanent delete: `POST /events/admin/{id}/force-delete` (reason required; any status; cascades); `/admin/events` row + bulk Delete permanently |
| 2026-08-02 | Admin events list lifecycle: `/admin/events` checkbox + bulk/row Deactivate (pause), Cancel, Archive; admin discard unused drafts |
