...
| Ambassadors (platform-wide) | `ambassadors` (`program_kind=platform_wide`, nullable `host_id`) | Admin enroll/invite | Owner me + admin | Suspend/end | Soft suspend/end | Soft preferred | `/r/{code}`, ambassador dashboard | `/promos/admin/referral-programs/*/enrollments`, `/promos/referral/resolve/{code}` | partial | One durable code; tickets+merch via rules; payer_type=platform on sales |
