...
| Resource | Model | Create | Read | Update | Delete / lifecycle | Hard delete? | Frontend | Backend (key) | Status | Primary gap |
|---|---|---|---|---|---|---|---|---|---|---|
| Blog posts | `blog_posts` (+ categories/tags/authors; studio: `studio_brief`/`studio_outline`/`faqs`/`content_version`; `blog_revisions`; `blog_ai_operations`) | Admin (`admin.blog.create`) | Public published only; admin all (+ draft preview) | PATCH + autosave (content_version conflict 409) | Publish / unpublish / schedule / archive (publish creates revision; AI never auto-publishes) | Prefer archive | `/blog*`, `/admin/blog*`, `/admin/analytics/blog` | `/api/v1/blog/*`, `/api/v1/admin/blog/*`, `/api/v1/admin/blog/ai/*`, `/api/v1/analytics/admin/blog*` | **complete** | Legacy `cms_blog_posts` migrated; markdown + sanitize; Blog AI Studio; first-party engagement + editorial analytics (`entity_type=blog_post`) |
